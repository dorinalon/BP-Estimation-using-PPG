...
# Returns row_count= number of rows in the csv, data= data vector.
def read_csv(path):
    i = 0
    row_count = 0
...
# Creates a systolic and diastolic BP of train VS test.
def calc_loss_precentage(expec_BP, pred_BP, time_length, patient_num, start_time, graph_name, figures_path):
...
# Train rnn and test. 
# Parameters:
# path_bp is the path of the BP csv file.
# path_ppg is the path of the PPG csv file.
# patient_num is the examined patient number.
# train_start_time is the time in minutes to start the training process.
# figures_path is a path where all the outcomes will be saved.
# rnn_path is the path where the rnn exist or will be saved.
# enableBatch is a boolean. if true- divide the train set to batches. if false- use train set as is.
# enableTrain is a boolean. if true- create a rnn and train it on the train set. if false- use an exsisting rnn.
def Train_and_Test(path_bp, path_ppg, patient_num, train_start_time, figures_path, rnn_path, enableBatch, enableTrain):
    ...
    if enableTrain:
        ...
        # Save the net
        torch.save(r.state_dict(), rnn_path)
        # Mean Loss
        plt.clf()
        plt.plot(loss_vec)
        plt.title("Mean loss")
        ...
        r.load_state_dict(torch.load(rnn_path))  # Loading the net
    ...
    calc_loss_precentage(test_bp[:, 0], pred_t[:, 0], test_size_125Hz, patient_num, train_start_time, "test on self", figures_path)
    ...
    calc_loss_precentage(test1_bp_segment[:, 0], pred_BP_test_1[:, 0], test_size_125Hz, patient_num, train_start_time, "test_on_1887524-1978_0-9min", figures_path) #TODO write headline
...
Train_and_Test(
    ...
    '2642420-8079',0, '/home/shirili/Downloads/ShirirliDorin/project_A/data_125Hz/2642420-8079/test_on_others',
	'/home/shirili/Downloads/ShirirliDorin/project_A/data_125Hz/2642420-8079/nn_16Sec_2642420-8079.pt', True, False)
